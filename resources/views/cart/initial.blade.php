        ...
        <section class="py-5">
            <div class="container px-4 px-lg-5 mt-5">
                ...
                @if (count($cart) > 0)
                    <table class="table">
                        ...
                        @foreach ($cart as $item)
                            ...
                            <tr>
                                ...
                                <td>
                                    <!-- عرض الكمية مع زر التحديث -->
                                    <form action="{{ route('cart.update') }}" method="POST" class="d-inline">
                                        @csrf
                                        <input type="hidden" name="product_id" value="{{ $item['product_id'] }}">
                                        <input type="number" name="quantity" value="{{ $item['quantity'] }}" min="1" class="form-control form-control-sm d-inline-block" style="width: 60px;">
                                        <button type="submit" class="btn btn-primary btn-sm">{{__('labels.Update')}}</button>
                                    </form>
                                </td>
                                <td>
                                    <!-- زر حذف المنتج -->
                    ...
                    <a href="{{ route('home') }}" class="btn btn-primary mb-5">{{__('menu.home')}}</a>
                @else
